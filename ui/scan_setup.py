...
class ScanSetupWidget(QWidget):
    ...
    show_pairs = Signal() # Load previous search results
    ...
    def __init__(self, session):
        ...
        # Show Pairs Button (load previous results)
        self.btn_show_pairs = QPushButton("Show Pairs")
        self.btn_show_pairs.setStyleSheet("font-size: 16px; font-weight: bold; padding: 10px; background-color: #225522;")
        self.btn_show_pairs.setToolTip("Load previous search results from database")
        action_layout.addWidget(self.btn_show_pairs)
        ...
        self.btn_show_pairs.clicked.connect(self.on_show_pairs)
    ...
    def on_show_pairs(self):
        """Load previous search results using current threshold settings."""
        # Update session with current threshold for filtering
        self.session.threshold = self.spin_thresh.value()
        ...
        logger.info(f"Loading previous pairs with threshold: {self.session.threshold}")
        self.show_pairs.emit()
